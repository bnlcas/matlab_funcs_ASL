function [] = plot_brain_elecs_sandbox_greycbar(dat)


%% Get Colormap:
% color_res = 500;
% gray_level = 0.5;         <---- Default color bar settings...
% [cmap, data_axis] = make_approx_cbar(dat);   <----- older version

% negative_rgb = [0.8 0.8 0];
% positive_rgb = [0 0.8 0.8];

% negative_rgb = [0.8 0.25 0.4];
% positive_rgb = [0 0.8 0.8];

negative_rgb = [0 0 0.8];
%positive_rgb = [0 0 0.8];
...
% 
               ...
               scatter3(elecmatrix(i,1)+offset,elecmatrix(i,2),elecmatrix(i,3),...
               elecSize, [0.5 0.5 0.5]); %   colorscaling3(i) colorscaling2(i)],'filled');
