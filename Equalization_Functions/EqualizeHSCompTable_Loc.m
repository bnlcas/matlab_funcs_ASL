...
handshapes(strcmpi(handshapes, 'lax')) = [];

hs_size = zeros(length(handshapes),1);
% Find biggest_hs
for i = 1:length(handshapes)
    hs_size(i) = sum(strcmpi(ERPs.grouped_annot.handshape(Data_Tag), handshapes(i)));
end
[~, order] = sort(hs_size,'descend');
handshapes = handshapes(order(1:10)); % takes the 10 largest categories
...
for i = 1:length(handshapes);
    ...
    for k = 1:length(loc_names)
        for j = (k+1):length(loc_names)
            ...
            if strcmpi(hs_name,'5') & strcmpi(loc_1,'face') & strcmpi(loc_2,'neutral')
                PlotECogGrid_Gen(ERPs, true, ERPs.ecog(:,:,Loc12_Comp_on_Loc(:,1)), ERPs.ecog(:,:,Loc12_Comp_on_Loc(:,2)))
            end
    ...
    if strcmpi(hs_name,'5')
        hs_name = 'Five';
    elseif strcmpi(hs_name,'K/P')
        hs_name = 'KP';
    elseif strcmpi(hs_name, '5>')
        hs_name = 'Bent_5';
    end
