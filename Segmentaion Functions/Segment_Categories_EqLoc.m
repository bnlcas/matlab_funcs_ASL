function [output] = Segment_Categories_EqLoc(ERPs, sig_chans, category_size_thresh, Analysis_frame)
...
%% Assemble list of relevant handshapes
...
categories_hs(strcmp(categories_hs,'lax')) = []; %kill LAX hs
categories_hs(strcmpi(categories_hs,'S')) = []; %kill S hs
categories_hs(strcmpi(categories_hs,'O')) = []; %kill O hs
class_names = handshapes;
cat_count = zeros(size(categories_hs));
for i = 1:length(categories_hs)
    cat_count(i) = sum(strcmp(handshapes, categories_hs(i)));
end
categories_hs(cat_count < category_size_thresh) = [];
cat_count(cat_count <  category_size_thresh) = [];

%% Find Intersection of common locations
locations = ERPs.grouped_annot.loc(Data_Tag);
locations = strrep(locations, ' ','');
categories_loc = unique(locations);
categories_loc(strcmp(categories_loc, '')) = []; % remove noncategorized locations
common_loc = categories_loc
for i = 1:length(categories_hs)
    hs_index = strcmpi(handshapes, categories_hs(i));      % get indexes of location category
    categories_loc_hs = unique(locations(hs_index));
    categories_loc_hs
    common_loc = intersect(common_loc, categories_loc_hs);    % get intersection of meaningful handshapes
end


%% Find Sizes of relevant handshapes (minmum size of a hs among all locations)
common_loc_sizes = zeros(size(common_loc));   % array of the minimum instances of each hs common to all locations.
loc_size = zeros(size(categories_hs));     % array of the number of instances of a hs in location i
for i = 1:length(common_loc)
    for j = 1:length(categories_hs)
        hs_index = strcmpi(handshapes, categories_hs(j));
        loc_hs = locations(hs_index);              % list of handshapes occuring in location_j
        hs_size(j) = sum(strcmpi(loc_hs, common_loc(i)));   % number of times 
    end
    common_loc_sizes(i) = min(hs_size);
end        


categories = categories_hs;
...
%%Shuffle the Order on Everything: ***NECESSARY FOR DOWNSAMPLING
shuffle_order = true;
if shuffle_order
...
for i = 1:length(categories)
    ...
    for j = 1:length(common_loc)
        %total_hs_j_in_cat = sum(strcmpi(common_hs(j), handshapes(in_category))); % Total Instances of Common_HS(j) in category i
        is_cat_i_and_hs_j = in_category & strcmpi(common_loc(j), locations);
        keep = find(is_cat_i_and_hs_j, common_loc_sizes(j));
        List_Keep(keep) = true;
    end
end
