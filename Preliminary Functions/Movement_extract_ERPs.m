...
function [ERPs]= Movement_extract_ERPs(datarootdir, subj, timeLim, zscore_timeLim, annot)
...
for i = 1:length(annot.start_time)
    allEventTimes(i).StartTime = annot.start_time(i);
end
allEventStartTimes = annot.start_time; %[allEventTimes.StartTime];
blocks=repmat({'B5'},length(allEventStartTimes),1);
current_block='B5';
...
Is_Stim = ~strcmpi(annot.Movement, ''); %% Used to EXTRACT PreStim Normalized ERPs
%Is_Stim = true(size(annot.Movement));
...
for i = 1:length(allEventStartTimes)% typically 600 trials for phoneme restoration task
    ...
    %%
        ...
        if i==length(allEventTimes)%or(strcmp(allEventTimes(1,i).block,current_block)==0,i==length(allEventTimes))% if the block has changed for this trial or we are at the end of allEventTimes
        ...
        badTrials = Find_bad_trials(datarootdir,subj,current_block,[allEventTimes(i-block_length+1:i).StartTime],timeLim);% calls Find bad trials function
