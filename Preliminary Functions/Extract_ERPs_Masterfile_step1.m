...
function [ERPs]= Extract_ERPs_step1(datarootdir, subj, timeLim, logfile_data, TransLex_OnDur_Times)
...
EventStartTimes = TransLex_OnDur_Times(:,1);
EventDurTimes = TransLex_OnDur_Times(:,2);
EventBlock = TransLex_OnDur_Times(:,3);


blocks={allEventTimes.block};
...
[ecogDS.data,ecogDS.sampFreq] = readhtks([datarootdir '/' subj '/' subj '_' current_block '/HilbAA_70to150_8band']);
...
if isempty(timeLim)
    timeLim=[-.500 1.5];% take 500 ms before and 1500 ms after event.
end
...
data.ecog = zeros(size(ecogDS.data,1),length(timeLim(1)*ecogDS.sampFreq:timeLim(2)*ecogDS.sampFreq),length(EventStartTimes));
standard_timesize = length(timeLim(1)*ecogDS.sampFreq:timeLim(2)*ecogDS.sampFreq);
...
% around stimuli
for i = 1:length(EventStartTimes)% typically 600 trials for phoneme restoration task
    StimStartTime=EventStartTimes(i);
    
%    round(allEventTimes(1,i).StartTime*ecogDS.sampFreq);

    datTimes = [max(round(StimStartTime + timeLim(1)*ecogDS.sampFreq),1),   round(StimStartTime + EventDurTimes(i))];% window around stim onset
    
    
        % looks at transition from one block to the next
        if or(strcmp(strcat('B',num2str(EventBlock(i))),current_block)==0,i==length(EventStartTimes))% if the block has changed for this trial or we are at the end of allEventTimes
        
        % Save information  collected during prior block
        badChans = textread([datarootdir '/' subj '/' subj '_' current_block '/Artifacts/badChannels.txt']);% Calls list of channels with artifacts identified during prelim Analysis (usually flat) 
    ...
    %% Normalize and Collect data
        ...
        Normalizing_data=Raw_data(:,1:round(abs(timeLim(1)*ecogDS.sampFreq)./2)); %takes average of the first half of the timewindow taken prior to the partitioning event
        ...
        buffersize = standard_timesize - size(Normalized_data,2);
        nan_buffer = NaN(size(Normalized_data,1),buffersize);
        Normalized_Buffered_data = [Normalized_data, nan_buffer];
        data.ecog(:,:,i) = Normalized_Buffered_data;
