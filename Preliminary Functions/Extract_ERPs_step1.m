...
function [ERPs]= Extract_ERPs_step1(datarootdir, subj, timeLim, logfile_data)
...
allEventStartTimes = [allEventTimes.StartTime];
...
for i = 1:length(allEventStartTimes)% typically 600 trials for phoneme restoration task
    ...
    %% Normalize and Collect data
        ...
        Normalizing_data=Raw_data(:,1:round(abs(timeLim(1))*ecogDS.sampFreq)); %takes average of the timewindow taken prior to the stimulus onset
